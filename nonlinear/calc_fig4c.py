...
def simul(K, width, batch_size, learning_rate, nepoch, seed):
    ...
    for i in range(nepoch):
        if nepoch <= 1000: #record test error frequently if nepoch is small
            for j in range( int(floor(n_train/batch_size)) ):
                perftmps = sess.run([base_error, update_ws, update_bs], feed_dict = train_dicts[j])
                train_error = perftmps[0]
                if (i == 0) or (i < 10 and j%10 == 0) or (j==0):
                    perftmps = sess.run([base_error, wnorms], feed_dict=base_dict)
                    b_error = perftmps[0]
                    w0tmp = perftmps[1][0]; w1tmp = perftmps[1][1]
                    fwe.write(str(i) + " " + str(j) + " " + str(b_error) + " " + str(train_error) + " " + str(w0tmp) + " " + str(w1tmp) + "\n"); fwe.flush()

        else: #record test error sporadically if enpoch is large
            perftmps = sess.run([base_error, wnorms], feed_dict=base_dict)
            b_error = perftmps[0]
            cm_b_error += 0.1*b_error
            w0tmp = perftmps[1][0]; w1tmp = perftmps[1][1]
            ...
            if i%10 == 0:
                if i == 0:
                    fwe.write(str(i) + " " + str(b_error) + " " + str(train_error) + " " + str(w0tmp) + " " + str(w1tmp) + "\n"); fwe.flush()
                else:
                    fwe.write(str(i) + " " + str(cm_b_error) + " " + str(train_error) + " " + str(w0tmp) + " " + str(w1tmp) + "\n"); fwe.flush()
                cm_b_error = 0.0
